...
tbl_locs <- readr::read_csv("Douglas Filtered/TtTag002-035_DouglasFiltered_KS_r20d3lc2_2020MAYv2.csv",
...
## subset Kauai TtTags 
tbl_locs <- tbl_locs %>%
  filter(animal %in% c("TtTag002","TtTag004","TtTag005","TtTag008","TtTag009","TtTag010",
                       "TtTag012","TtTag013","TtTag019","TtTag020","TtTag022","TtTag023",
                       "TtTag025","TtTag034","TtTag035"))


length(unique(tbl_locs$animal)) # 15 kauai tags 
...
# fit model
tbl_data <- tbl_data %>%
  dplyr::mutate(fit = purrr::map(data, ~fit_func(d = .x, fixPar = fixPar, prior = prior,
                                                 constr = constr)))

# can check parameter estimates by
tbl_data[[3]][[15]] # example, first tag 
...
tbl_data %>%
  ...
  st_intersects(islands) %>%
  purrr::map_lgl(~ length(.x) > 0) %>%
  sum() # all land locs removed, some segments still crossing over land 
...
## write csv
write.csv(final, "PMRF Feb 2020 Report/TtTags_Kauai_crawl_1hrStep_FixedPath_2020Sep28.csv", row.names = F)
