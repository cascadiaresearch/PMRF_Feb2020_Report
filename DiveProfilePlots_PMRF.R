...
Tag = "TtTag035"
behavior <- read.csv("Dive behavior/TtTag035_175452-Behavior_corrected.csv", header=T, stringsAsFactors = F)
...
LocsFile <- read.csv("Douglas Filtered/TtTag002-035_DouglasFiltered_KS_r20d3lc2_2020MAYv2.csv", header = T)
locs <- filter(LocsFile, animal == "TtTag035") # filter out tag (Argos only locs here)
...
# TtTag034
TtTag034.all = DivePlotAll(behavior, title = "TtTag034 Complete", gaps = F, plotcolor = "blue", depthlim = -1000,
                           datebreaks = "1 day", nights = nights, filetz = "UTC", grid = F,
                           gapcolor = "black")


TtTag034.all =  TtTag034.all + theme(axis.text=element_text(size=11, colour = "black"),
                                     axis.title=element_text(size=12)) + xlab("")
TtTag034.all

ggsave(paste0(plot_folder, Tag, "_AllDives_2020Oct2.jpg"), TtTag034.all, device = "jpeg", units = "in", width = 8, height = 4, dpi = 300)

### plot dives for each 24 hour period ##
# change the datelim = to the two dates between 24 hour period
TtTag034.1 = DivePlotDay(behavior, title = "TtTag034, 2/24 - 2/25", gapcolor = "black", plotcolor = "blue", depthlim = -1000,
                         datebreaks = "2 hour", nights = nights,
                         datelim = as.POSIXct(c("2020-02-24 12:00:00", "2020-02-25 12:00:00"), tz = "Pacific/Honolulu"),
                         filetz = "UTC",
                         gaps = F,
                         grid = F, lineweight = 0.3)

TtTag034.1 = TtTag034.1 + theme(axis.text.y=element_text(size=8, colour = "black"),
                                axis.title=element_text(size=12),
                                axis.text.x = element_text(size = 8, colour = "black"))

TtTag034.1 = TtTag034.1 + xlab("Time (HST)")

TtTag034.1

ggsave(paste0(plot_folder, Tag, "_Dives", "2020FEB24-25.jpg"), TtTag034.1, device = "jpeg", units = "in", width = 8,
       height = 4, dpi = 300)

# TtTag035
TtTag035.all = DivePlotAll(behavior, title = "TtTag035 Complete", gaps = F, plotcolor = "blue", depthlim = -1000,
                           datebreaks = "1 day", nights = nights, filetz = "UTC", grid = F,
                           gapcolor = "black")
...
ggsave(paste0(plot_folder, Tag, "_AllDives_2020Oct2.jpg"), TtTag035.all, device = "jpeg", units = "in", width = 8, height = 4, dpi = 300)

### plot dives for each 24 hour period ##
# change the datelim = to the two dates between 24 hour period
TtTag035.1 = DivePlotDay(behavior, title = "TtTag035, 2/24 - 2/25", gapcolor = "black", plotcolor = "blue", depthlim = -1000,
                         datebreaks = "2 hour", nights = nights,
                         datelim = as.POSIXct(c("2020-02-24 12:00:00", "2020-02-25 18:00:00"), tz = "Pacific/Honolulu"),
                         filetz = "UTC",
                         gaps = F,
                         grid = F, lineweight = 0.3)
...
ggsave(paste0(plot_folder, Tag, "_Dives", "2020FEB24-25.jpg"), TtTag035.1, device = "jpeg", units = "in", width = 8,
       height = 4, dpi = 300)
