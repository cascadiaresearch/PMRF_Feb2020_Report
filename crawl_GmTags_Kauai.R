...
#             model to animal movement data to 
#                     Kauai Gm tags
...
## import Douglas filtered locations, Argos + GPS locations (for GmTag231)
tbl_locs <- readr::read_csv("PMRF Feb 2020 Report/Kauai_GmTags_ArgosGPS_DougFilt_2020Oct06.csv",
...
tbl_locs$LC <- factor(tbl_locs$LC, levels = c("DP","L3","L2","L1","L0","LA","LB","LZ", "G"))
summary(tbl_locs$LC)

## filter western community residents  
tbl_locs <- tbl_locs %>%
  filter(animal != "GmTag078") %>%
  filter(animal != "GmTag104") %>%
  filter(animal != "GmTag117") %>%
  filter(animal != "GmTag152") %>%
  filter(animal != "GmTag153")
...
tbl_locs <- tbl_locs %>%
  mutate(
    smaj = ifelse(lc %in% c("DP","G"), 50, smaj),
    smin = ifelse(lc %in% c("DP","G"), 50, smin),
...
constr <- list(lower = c(-Inf, -3), upper = c(Inf, 4))

fit_func <- function(d, fixPar, prior, constr) {
  suppressWarnings(
    crwMLE(mov.model = ~ 1,
           err.model = list(
             x = ~ ln.sd.x + 0,
...
set.seed(123)
tbl_data <- tbl_data %>%
  dplyr::mutate(fit = purrr::map(data, ~fit_func(d = .x, fixPar = fixPar, prior = prior, constr = constr)))

# can check parameter estimates by
tbl_data[[3]][[4]] # example, first tag 

## predict locations at 1 hour time interval
tbl_data <- tbl_data %>% 
  dplyr::filter(map_lgl(fit, ~inherits(.x,"crwFit"))) %>% 
  dplyr::mutate(pred_pts = purrr::map(fit, 
                                      ~crwPredict(.x, predTime = "1 hour", return.type = 'flat')))
...
## fix paths 
islands <- ptolemy::extract_gshhg(sf_locs, buffer = 20000, epsg = 3750, resolution = "h") # extract polygon data for region of loc data
plot(islands)
...
tbl_data %>%
  ...
  unnest() %>%
  st_as_sf(crs = 3750) %>%
  st_intersects(islands) %>%
  purrr::map_lgl(~ length(.x) > 0) %>%
  sum() # 169 points on land 

## re-route portions of predicted paths around land
tbl_data <- tbl_data %>%
...
## create sf objects of fixed paths 
tbl_data <- tbl_data %>%
  mutate(
...
tbl_data %>%
  ...
  mapview(map.types = "Esri.OceanBasemap", zcol = "animal",
          burst = TRUE, legend = FALSE, homebutton = FALSE) ## a couple of segments for TtTag034 cross overland
...
final <- pred_data %>%
  select(animal, date, locType, latitude, longitud, se.mu.x, se.mu.y, -geometry)


## write csv
write.csv(final, "PMRF Feb 2020 Report/Kauai_Pelagic_GmTags_crawl_1hrStep_FixedPath_2020Oct06.csv", row.names = F)
